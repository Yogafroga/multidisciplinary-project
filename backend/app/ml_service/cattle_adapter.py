...
class CattleAdapter:
    ...
    def predict_sync(self, image_bytes: bytes):
        """Синхронное предсказание из байтов изображения"""
        return self.predictor.predict_from_bytes(image_bytes)

    async def predict_async(self, image_bytes: bytes):
        """Асинхронное предсказание из байтов изображения"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(self.executor, self.predict_sync, image_bytes)
